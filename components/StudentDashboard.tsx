...
const PDF_STYLES_COLOR = `
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; }
    ...
    .header-box { display: flex; align-items: center; margin-bottom: 12px; border-bottom: 3px solid #1e1b4b; padding-bottom: 12px; position: relative; }
    ...
    .address { font-size: 10px; color: #1e1b4b; font-weight: 800; text-transform: uppercase; background: #fff7ed; border-top: 1.5px solid #1e1b4b; border-bottom: 1.5px solid #1e1b4b; padding: 3px 0; }
    ...
    .student-info { margin-top: 15px; margin-bottom: 15px; font-size: 12px; background: #fff1f2; padding: 12px; border-radius: 10px; border: 1.5px solid #e11d48; display: grid; grid-template-columns: 1fr 1fr; gap: 8px 20px; }
    .info-item { display: flex; align-items: center; }
    .info-label { font-weight: 900; width: 150px; color: #e11d48; text-transform: uppercase; font-size: 10px; }
    .info-value { font-weight: 800; color: #1e1b4b; border-bottom: 1px dashed #e11d48; flex: 1; padding-left: 5px; }

    .tables-flex { display: flex; gap: 15px; margin-bottom: 15px; }
    .semester-table-box { flex: 1; border: 1.5px solid #1e1b4b; border-radius: 8px; overflow: hidden; }
    .table-title { font-weight: 900; text-align: center; padding: 6px; font-size: 11px; text-transform: uppercase; background: #1e1b4b; color: #fff; }
    ...
    .remarks-title { font-weight: 900; text-align: center; background: #1e1b4b; color: #fff; padding: 6px; font-size: 11px; text-transform: uppercase; }
    .remarks-table { width: 100%; border-collapse: collapse; }
    .remarks-table th { background: #fefce8; color: #854d0e; font-size: 9px; padding: 4px; border: 1px solid #1e1b4b; }
    .remarks-table td { height: 50px; vertical-align: top; padding: 8px; font-size: 11px; font-weight: 500; border: 1px solid #1e1b4b; text-align: left; line-height: 1.3; }

    .grade-key-box { margin-bottom: 15px; border: 1.5px solid #1e1b4b; border-radius: 6px; overflow: hidden; }
    .key-header { background: #fef2f2; color: #e11d48; padding: 4px 10px; font-size: 10px; font-weight: 900; border-bottom: 1px solid #1e1b4b; }
    .key-grid { display: grid; grid-template-columns: repeat(8, 1fr); font-size: 10px; font-weight: 800; text-align: center; }
    .key-cell { border-right: 1px solid #1e1b4b; padding: 5px 2px; }
    ...
    .footer-line { font-weight: 900; margin-bottom: 4px; text-transform: uppercase; font-size: 13px; color: #1e1b4b; }
    .footer-line span { color: #e11d48; }
    
    .signatures { display: flex; justify-content: space-between; margin-top: 35px; padding: 0 15px; }
    .sig-box { text-align: center; border-top: 2px solid #1e1b4b; width: 180px; padding-top: 8px; font-size: 12px; font-weight: 900; color: #1e1b4b; }
`;
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const generatePDFContent = () => {
    ...
    const s1Rows = subjects.map((sub, i) => `<tr><td style="width:35px;">${i+1}</td><td class="subject-cell">${sub}</td><td class="grade-val" style="width:55px;">${record.sem1Grades?.[sub] || '-'}</td></tr>`).join('');
    const s2Rows = subjects.map((sub, i) => `<tr><td style="width:35px;">${i+1}</td><td class="subject-cell">${sub}</td><td class="grade-val" style="width:55px;">${record.sem2Grades?.[sub] || '-'}</td></tr>`).join('');
      ...
      <div class="report-wrapper">
        <div class="report-page">
            ...
            <div class="student-info">
                <div class="info-item"><span class="info-label">Name of Student :</span><span class="info-value">${student.name.toUpperCase()}</span></div>
            ...
            <div class="remarks-wrapper">
                ...
                <table class="remarks-table">
                    <thead>
                        <tr><th colspan="3" style="background:#fff1f2;">1st Semester Performance</th><th colspan="3" style="background:#ecfdf5;">2nd Semester Performance</th></tr>
                        <tr>
            ...
            <div class="grade-key-box">
                <div class="key-header">Grade Key & Evaluation Criteria</div>
                <div class="key-grid">
            ...
            <div class="result-footer">
                <div class="footer-line">RESULT : <span>${record.resultStatus || 'PASS'}</span></div>
                <div class="footer-line">NEXT YEAR STANDARD : <span>${nextClass}</span></div>
                <div class="footer-line">SCHOOL REOPENING : 11th JUNE 2025</div>
                <div class="signatures">
                    <div class="sig-box">TEACHER</div>
              ...
              <div className="space-y-6 animate-in fade-in zoom-in-95">
                  ...
                  {studentAnnualRecord ? (
                      <div className="bg-slate-200 p-4 sm:p-12 rounded-3xl border shadow-inner flex justify-center items-start overflow-x-auto">
                          <div className="bg-white shadow-2xl scale-75 sm:scale-95 origin-top min-w-[210mm]" dangerouslySetInnerHTML={{ __html: `<style>${PDF_STYLES_COLOR}</style>${generatePDFContent()}` }} />
                      </div>
                  ) : (
